...
**Category-page length rethink — lean-hub prototype (accommodation, 2026-06-05):** measured the bloat — the 7 per-sub-category editorial sections were **53% of the page** (2,985px of 5,615px) and don't scale (only 7 of ~14+ sub-cats). Prototype: **non-destructively hid the editorial sections** (`.main-col > .section{display:none}`) → page **5,615px → 2,941px (−48%)**. Lean hub = hero + search + visual tile grid (the nav) + CTA + FAQ + local-info sidebar. The FAQ already restates the sub-category info in Q&A form, so AEO value is retained; the deep prose should **migrate to the sub-category pages** (better SEO — no cannibalisation) in the final pass. REVERTED per user — lean hub (removing editorial) not the chosen direction. User wants to KEEP the editorial content but solve length via progressive disclosure (accordion or tabbed). Reconsidering.

**Category redesign — image-led feature rows (accommodation, 2026-06-05):** after the user rejected lean-hub and confirmed *images are the biggest benefit*, rebuilt the 7 editorial sections from stacked text cards (15 cards) into **single image-led feature rows** (photo/branded-placeholder panel + tight 1-2 line summary + Browse chips for each sub-cat). Solves all three problems at once: imagery (Carwyn), visual rhythm/scannability, and length (**5,615px → 4,140px, −26%**) — while keeping every link + combined `data-tags` so search still works. 3 real photos (Pacific Park aerial, styled interior, Karaka Pines → optimised webp in `assets/category-features/`) + 4 brand-gradient placeholders (icon centred) where we don't have sub-cat photos yet. Deployed to accommodation. **Next:** roll out to other 4 + (optional) sub-category photos to replace the 4 placeholders.

**Editorial neutrality — no specific businesses in general sections (accommodation, 2026-06-05):** per user, general category copy shouldn't name specific businesses (that's what listings are for; naming some favours them). Genericised the 6 feature-row summaries + the sidebar (AIMS box, booking tips) + an image alt — removed Pacific Park, Tasman, Beyonder, Beach House, Bay Palm, Energy, The Reef, Belle Mer, Karaka Pines, Eves, Ray White, Bookabach/Holiday Houses. Kept `data-tags` (invisible search aids). Done: FAQ genericised too — all 6 business-naming answers rewritten in both the visible accordion and the FAQPage JSON-LD (user chose full neutrality). 0 visible business names on the page; only lowercase data-tags keep them as search keywords. To apply site-wide to the other 4 category pages when their sections are rebuilt.
