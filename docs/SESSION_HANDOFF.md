...
3. **★ NEXT — the subcategory-page structural pass** (the remaining Bronze surfacing + a known carry-over). Give every subcat page a consistent **Gold (full-width) → Silver (full-width) → Bronze (small cards, = "More [subcat]" section) → free-listing CTA** stack. Bronze tier is already wired (nav.js injects live listings into `.ghost-grid`, no claim link). To do: (a) the **empty-state "claim this spot / add your business"** card for subcats with no grid/listings; (b) **roll the "More [subcat]" section onto every subcat page** (only ~13 have a `.ghost-grid` today; the rest need it added — overlaps the §18 subcat-polish task: old accents, width/CSS/nav/footer consistency); (c) the real **Gold/Silver listing tiers** on subcat pages (currently only the ghost-grid exists); (d) eventually **drop the fake placeholder ghosts**. This is a sizable, somewhat repetitive sweep across ~100+ inconsistent subcat pages — scope it first.
4. **Self-serve Spotlights / Ad Spots** — Phase 2 (Phase 1 = Silver/Gold claim open slots). §17.11.
5. **Bronze lead-funnel nurture** — owner go-live email is wired (needs `RESEND_API_KEY` set on the worker + a verified `papamoa.info` sender in Resend); full Day 0/3/6/10 nurture = §18 Phase E (Brevo).
