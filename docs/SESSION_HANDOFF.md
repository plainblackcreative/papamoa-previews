...
9. ✅ **Bronze nurture runbook — WRITTEN (session 7 extended, `925238f`).** See `docs/bronze-nurture-resend-runbook.md`. Two paths around the DNS blocker; Path A gets emails firing today via plainblack.com sender. Worker code already wired; just needs the Resend API key + sender swap.
...
11. **★ NEXT real work options.** (a) **Footer migration tranche 2 — 29 community pages still on site-footer.** Each likely needs a contextual column 3 (related community pages by topic). ~1-2h. (b) **Carwyn-decisions session** — lock §17.13 + §17.14, then send the reply. (c) **Bronze nurture wire-up** — execute the runbook (Path A, ~30 min Jay work). (d) **DNS consolidation session with Carwyn** — unblocks Path B for Resend + Brevo + future. (e) **Phase A2 "How this site works" operator guide** for Carwyn dashboard. (f) **Phase D CRM dashboard rebuild** through worker. (g) **Phase E Brevo Day 0/3/6/10 sequences.**
