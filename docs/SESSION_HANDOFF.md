...
2. **Bronze self-serve auto-create — v1 BUILT (session 5), pending deploy.** Architecture + full deploy checklist in **§8.2**. Built: `listing-bronze-template.html` (①); `sales/create-bronze-listing.html` form + live preview (②, works today via pb-forms); `worker.js` routes `/bronze-submit` `/bronze-public` `/bronze-list` `/bronze-approve` `/bronze-reject` (③④); `admin/bronze-queue.html` moderation UI (④); `assets/bronze-cards.js` + slots on the 5 category pages surfacing live listings (⑤); owner go-live upgrade email (⑤ lead-funnel first touch). **NOT deployed / untested end-to-end** — Jay provisions the Google Sheet + GitHub `contents:write` PAT + `BRONZE_ADMIN_TOKEN` + `wrangler deploy`, then swaps the form `SUBMIT_URL` (all steps in §8.2). Expect to debug the first live run (service-account Sheet write + GitHub commit are the likely spots). Full Brevo nurture sequence = §18 Phase E.
