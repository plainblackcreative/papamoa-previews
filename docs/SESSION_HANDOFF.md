...
**Last commit:** `4ddc71e` spotlight: §17.11 Phase 1A -- wire Claim CTAs through contact modal
...
15. **★ NEXT real work options** (sequencing reflects Jay's "site has alot more work" framing):
    ...
    - (c) **Legacy inline contact-modal sweep.** 116 pages still carry an inline `<div class="pnf-overlay">` + inline pnfOpenContact/pnfCloseContact/pnfSubmitContact. Phase 1A patched the inline function to accept `(type, ctx)` but the architecture says nav.js should own the modal entirely. Removing the inline modals lets nav.js inject its own (which has the enquiry-type dropdown). ~30 min Python sweep.
    ...
    - (h) **Site-wide Carwyn brand-rule audit** -- with tier-positioning + PPP retirement now landed, time to re-run the Carwyn req #1-#8 audit on what's actually shipped vs what the reply prep claims.
