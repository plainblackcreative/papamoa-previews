...
**Last commit:** `d862be0` copy: §3 scarcity-language sweep -- detangle tiers from Spotlight Ad Spot
...
13. **★ NEXT real work options** (sequencing reflects Jay's "site has alot more work" framing):
    - (a) **§17.11 Ad-Spots become load-bearing** -- the duration-based self-serve purchase flow + availability/impressions dashboard now move from Phase-2 deferred to in-progress, since scarcity revenue lives here. Build: per-sub-cat Spotlight availability + impressions counter, Stripe duration-based purchase (1/3/6/12mo), "claim/buy this spot" surface on each sub-cat page. Significant new build.
    - (b) **Footer migration tranche 2 -- 29 community pages still on site-footer.** Each likely needs a contextual column 3 (related community pages by topic). ~1-2h.
    - (c) **Phase A2 "How this site works" operator guide** for Carwyn dashboard.
    - (d) **Phase D CRM dashboard rebuild** through worker.
    - (e) **Phase E Brevo Day 0/3/6/10 sequences** -- gated on DNS sorted post-Carwyn.
    - (f) **Site-wide section audit** -- with tier-positioning now coherent, time to re-run the Carwyn brand-rule audit (Carwyn req #1-#8) on what's actually shipped vs what the reply prep claims.
