...
**Last updated:** 2026-06-04 (session 6) by Jay + Claude
**Last commit:** `2ae9aee` bronze: remove redundant CTAs from subcat pages
...
4. **★ NEXT — hero accent color fix.** 121 food-drink/shops/activities subcat pages still use the old brown/orange hero gradient + `--eat-accent` instead of the locked dark navy + blue (`#359FE8`). Scripted sweep ready to run — just needs the go-ahead.
5. **Self-serve Spotlights / Ad Spots** — Phase 2 (Phase 1 = Silver/Gold claim open slots). §17.11.
6. **Bronze lead-funnel nurture** — owner go-live email is wired (needs `RESEND_API_KEY` set on the worker + a verified `papamoa.info` sender in Resend); full Day 0/3/6/10 nurture = §18 Phase E (Brevo).
