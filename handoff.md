...
- **Photos: ALL 35 feature rows now have a real photo** (commit `a0e4a51`). Every `sa-photo-ph` gradient placeholder was swapped for a webp in `assets/category-features/` (`{page}-{group}.webp`), `role="img"` + generic alt. User-supplied/curated; 3 Unsplash gap-fills; no business signage/logos (hardware-store signage blurred). 28 source JPGs + `MANIFEST.txt` left untracked in `assets/category-features/`. Prompts/filenames in [docs/category-photo-prompts.md](docs/category-photo-prompts.md). *Minor: `food-takeaways.webp` reads a little pale — candidate for a swap.*
